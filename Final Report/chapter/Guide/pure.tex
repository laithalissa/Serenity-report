...
In simple terms, then, a free monad is a monad that can be generated directly from a functor ``for free'' --- ie without any additional information being given or imposed. This is achieved by maintaining the structure of the operations (as a list or stream) without actually interpreting them. A monadic value in this free monad can then be interpreted, ie converted to a monadic value in another monad by imposing a certain semantics, at any time at a later date, and even in multiple ways at different times.
...
\functions(liftFF, push, pop, sum, diff, mult, dup, example, interpret)
\begin{haskell}
...
>example = do push 9; push 5; mult; dup; mult; push 9; diff
...
\end{haskell}
\noindent
The "interpret" function can now be used to coerce a value of type "AbstractMachine a" (the free monad) into a full monadic type with a specific semantics. Three example semantics are shown in Listing~\ref{list:freesemantics}, firstly the normal operation of "StackMachine" with failure as implemented before, secondly some basic logging as a writer monad transformer, and lastly both of these simultaneously. 
...
\functions(asSM, asLog, asLogAndSM, tell)
\begin{haskell}
>asSM :: Num a => FreeFunctor (Operation a) c -> StackMachine a c
>asSM (FreeFunctor (Push a) c) = do pushM a; return c
>asSM (FreeFunctor Pop c)  = do popM; return (); return c
>asSM (FreeFunctor Summ c)  = do a <- popM; b <- popM; pushM (a+b); return c
>asSM (FreeFunctor Diff c) = do a <- popM; b <- popM; pushM (b-a); return c
>asSM (FreeFunctor Mult c) = do a <- popM; b <- popM; pushM (a*b); return c
>asSM (FreeFunctor Dup c)  = do a <- popM; pushM a; pushM a; return c

>asLog :: (Monad m, Show a) => FreeFunctor (Operation a) c -> WriterT String m c
>asLog (FreeFunctor (Push a) c) = do tell $ "Pushing " ++ show a ++ "\n"; return c
>asLog (FreeFunctor Pop c)  = do tell "Popping"; return c
>asLog (FreeFunctor Summ c)  = do tell "Summing\n"; return c
>asLog (FreeFunctor Diff c) = do tell "Subtracting\n"; return c
>asLog (FreeFunctor Mult c) = do tell "Multiplying\n"; return c
>asLog (FreeFunctor Dup c)  = do tell "Duplicating\n"; return c

>asLogAndSM :: (Show a, Num a) => FreeFunctor (Operation a) c -> WriterT String (StackMachine a) c
>asLogAndSM ff = do lift $ asSM ff; asLog ff
