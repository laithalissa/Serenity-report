...
\begin{center}
    ...
    location & used when the ship's location has changed & entityID, location \\ \hline
    direction & used when the ship's direction has changed & entityID, direction \\ \hline
...
The message has 5 fields:
\begin{description}
\item[shipID] integer field representing ship's unique id
\item[x] float field representing ship's x-coordinate
\item[y] float field representing ship's y-coordinate
\item[direction x] float field representing ship's x direction. Used in tandem with direction-y field.
\item[direction y] float field representing ship's y direction. Used in tandem with direction-x field.
\end{description}

There are a varying number of different update messages depending on what part of the game is being updated:
